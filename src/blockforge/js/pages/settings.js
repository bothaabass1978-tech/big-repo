...
(function (BF) {
  ...
  function gameplayTab(s) {
    ...
    return '<div class="settings-card"><h3>Audio</h3><div class="setting-row"><span class="sr-text"><span class="sr-label">Master volume</span><span class="sr-hint" id="vol-label">' + Math.round(g.volume * 100) + '%</span></span><input type="range" class="range" id="gameplay.volume" min="0" max="1" step="0.05" value="' + g.volume + '" style="max-width:220px"></div>' +
      ...
      '<div class="settings-card"><h3>Graphics</h3>' + seg('gameplay.graphics', g.graphics || 'auto', [['auto', 'Auto'], ['high', 'High'], ['low', 'Low'], ['classic', 'Classic 2D']], 'Game graphics',
        BF.g3d && !BF.g3d.supported() ? 'This device cannot run WebGL, so games use the Classic 2D renderer.' : 'Games and avatars render in 3D. Low turns off shadows and lowers resolution for older devices; Auto picks Low on small screens. Classic 2D uses the original flat renderer.') + '</div>' +
