  ...
  BF.pages.register('game', {
    ...
    render(params) {
      ...
      return '<section class="game-hero">' +
        ...
        '<div class="gh-stats"><div><span class="faint">Playing</span><b class="num"><span class="live-dot"></span> <span data-live="playing:' + g.id + '">' + U.fmt(st.playing) + '</span></b></div><div><span class="faint">Visits</span><b class="num">' + U.compact(st.visits) + '</b></div><div><span class="faint">Favorites</span><b class="num">' + U.compact(st.favorites) + '</b></div><div><span class="faint">Created</span><b>' + U.fmtDate(g.createdAt) + '</b></div><div><span class="faint">Updated</span><b>' + U.fmtDate(g.updatedAt) + '</b></div><div><span class="faint">Creator earned</span><b class="num" data-tip="' + esc('About ' + U.fmt(Math.round(BF.creatorEconomy.gameRevenue(g).perMin)) + ' ForgeCoins a minute right now') + '">' + BF.coinIcon(13) + ' ' + U.compact(BF.creatorEconomy.gameRevenue(g).lifetime) + '</b></div></div>' +
